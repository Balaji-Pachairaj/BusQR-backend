const { Motor_Unit } = require("../models/motor_unit_model");

const post_add_unit = async (req, res, next) => {
  try {
    let body = req.body;
    let response = await Motor_Unit.create(body);
...
const edit_unit = async (req, res, next) => {
  try {
    ...
    let response = await Motor_Unit.findByIdAndUpdate(body._id, body, {
      new: true,
    });
...
const authenticate_unit = async (req, res, next) => {
  try {
    ...
    // Find user by email
    let user = await Motor_Unit.findOne({ email });
...
const get_all_unit = async (req, res, next) => {
  try {
    let users = await Motor_Unit.find({}, "-password"); // Exclude password field for security
